...
def parser():
    ...
    p = commands.add_parser("run", help="主入口：连续bin+可选微震bin+岩爆时间清单，一次完成训练")
    p.add_argument("--continuous-dir", required=True)
    p.add_argument("--microseismic-dir", help="可选：外部识别程序输出的微震片段bin目录；不提供则关闭微震特征")
    ...
    p.add_argument("--continuous-dir", required=True)
    p.add_argument("--microseismic-dir", help="可选：需与训练模型的微震特征模式一致")
    ...
    try:
        if args.command == "run":
            ...
            result = run_workflow(args.continuous_dir, args.microseismic_dir, args.labels,
                                  ...
                                  args.min_validation_recall, args.min_active_bins)
