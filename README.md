...
准备岩爆前1小时的连续bin，模型每次使用最近30分钟连续波形和过去24小时微震片段，预测未来5、10、30分钟内发生岩爆的概率。**每完成一个bin推进一次，不固定按一分钟推进。**

**只需要两个bin目录和一份真实岩爆时间清单。不用手填4张CSV，程序自动生成内部索引、特征和标签。**
...
准备以下目录和文件；两个bin目录支持递归扫描。连续数据可以按“一个事件一个子文件夹”存放，每个子文件夹包含岩爆前后约1小时的连续bin：
...
例如在Windows上的目录`G:\1A岩爆预测\岩爆数据集\5.10.8.49`，WSL中通常对应`/mnt/g/1A岩爆预测/岩爆数据集/5.10.8.49`。WSL示例命令如下（微震识别程序输出目录请替换成实际路径）：
...
程序也会在WSL/Linux中把传入的`G:\\...`盘符路径尝试转换为`/mnt/g/...`。微震文件仍需通过`--microseismic-dir`单独提供；微震文件名不能替代人工确认的岩爆标签。
...
默认使用Sundial、30分钟连续历史、24小时微震历史。CPU运行把`--device cuda`改成`--device cpu`。
...
不需要`--labels`。backend和生成样本数从训练好的模型读取；训练时选了监测通道，这里传入相同的`--monitor`。
