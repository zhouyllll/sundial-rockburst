...
输入最近最多1小时连续波形和过去24小时由外部程序检出的微震片段，预测未来5、10、30分钟内发生岩爆的概率。
...
默认使用Sundial、60分钟连续历史、24小时微震历史。CPU运行把`--device cuda`改成`--device cpu`。

需要30分钟连续历史或指定监测通道时追加`--history-minutes 30 --monitor 15-34`。
...
3. 每分钟构造一次预测样本，取最近30/60分钟连续特征和前24小时微震统计。
